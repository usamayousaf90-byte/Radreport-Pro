...
/* ══════════════════════════════════
   AI CALL — with 429 retry + backoff
══════════════════════════════════ */
...
/* ══════════════════════════════════
   MAIN APP
══════════════════════════════════ */
export default function RadReport() {
  ...
  var expandField = useCallback(async function(sl, field) {
    ...
    if (r.ok) { setF(sl, field, r.text); showToast("✨ Expanded", "success"); }
  ...
  var expandSection = useCallback(async function(sec) {
    ...
    if (r.ok) {
      try {
        ...
        showToast("✨ Expanded " + count + " fields in " + sec.label, "success");
  ...
  var generateImpression = useCallback(async function() {
    ...
    if (r.ok) { setImpression(r.text); showToast("✨ Impression generated", "success"); }
    else showToast("AI error: " + r.error, "error");
  ...
  var expandImpression = useCallback(async function() {
    ...
    if (r.ok) { setImpression(r.text); showToast("✨ Expanded", "success"); }
    else showToast("AI error: " + r.error, "error");
    setLD("impExp", false);
  ...
  /* ══ SAVED DRAFTS ══ */
  var persistDrafts = function(_list) { /* in-memory only */ };

  var saveDraft = useCallback(function(label) {
    var name = (label || "").trim() || (patient.name ? patient.name + " — " + (modality||"") + " / " + (region||"") : (modality||"Report") + " / " + (region||""));
    var draft = {
      id: Date.now(),
      label: name,
  ...
  }, [patient, modality, region, findings, tags, impression, recommendation, urgency, showToast]);
  ...
  var loadDraft = useCallback(function(draft) {
    ...
    setStep("template");
    showToast("📂 Loaded: "" + draft.label + """, "success");
  }, [showToast]);

  var deleteDraft = useCallback(function(id) {
    setSavedReports(function(prev) {
      var next = prev.filter(function(r){ return r.id !== id; });
      persistDrafts(next);
      return next;
    });
    showToast("🗑️ Draft deleted", "info");
  }, [showToast]);

  var updateDraftLabel = useCallback(function(id, newLabel) {
    setSavedReports(function(prev) {
      var next = prev.map(function(r){ return r.id === id ? Object.assign({}, r, {label: newLabel}) : r; });
      persistDrafts(next);
      return next;
    });
  }, []);
  ...
  /* ══ SAVE DRAFT MODAL ══ */
  var SaveDraftModal = saveModalOpen ? (
    <div style={{position:"fixed",inset:0,zIndex:9000,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(4,9,15,.75)",backdropFilter:"blur(6px)"}}>
      <div style={{background:"#fff",borderRadius:16,padding:"32px 36px",width:"100%",maxWidth:420,boxShadow:"0 24px 64px rgba(0,0,0,.35)",fontFamily:"'DM Sans',sans-serif",animation:"slideUp .25s ease"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:20,color:"#0D2137"}}>💾 Save Draft</div>
            <div style={{fontSize:12,color:"#5A7090",marginTop:3}}>Name this report to find it easily later</div>
          </div>
          <button onClick={function(){setSaveModalOpen(false);setDraftLabel("");}} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:"#5A7090",lineHeight:1}}>×</button>
        </div>
        <label style={{fontSize:11,fontWeight:700,color:"#5A7090",textTransform:"uppercase",letterSpacing:.9,display:"block",marginBottom:6}}>Draft Name</label>
        <input
          autoFocus
          style={{width:"100%",padding:"10px 14px",border:"2px solid #DDE5EF",borderRadius:8,fontSize:14,color:"#1A2B3C",fontFamily:"'DM Sans',sans-serif",outline:"none",boxSizing:"border-box",marginBottom:8}}
          placeholder={(patient.name ? patient.name + " — " : "") + (modality||"Report") + " / " + (region||"")}
          value={draftLabel}
          onChange={function(e){setDraftLabel(e.target.value);}}
          onKeyDown={function(e){ if(e.key==="Enter") saveDraft(draftLabel); }}
        />
        <div style={{fontSize:11,color:"#8BA0B8",marginBottom:20}}>💡 Leave blank to auto-name from patient &amp; modality</div>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <button onClick={function(){setSaveModalOpen(false);setDraftLabel("");}} style={{padding:"9px 20px",borderRadius:8,border:"2px solid #DDE5EF",background:"transparent",color:"#5A7090",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
          <button onClick={function(){saveDraft(draftLabel);}} style={{padding:"9px 24px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#4F46E5,#7C3AED)",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 14px rgba(79,70,229,.4)"}}>💾 Save Draft</button>
        </div>
      </div>
    </div>
  ) : null;
  ...
  if (step === "saved") {
    ...
    var modalityColors = {
      "Ultrasound":"#0077B6","X-Ray":"#047857","CT Scan":"#B45309","MRI":"#6D28D9",
      "Gynaecology":"#BE185D","Mammography":"#7C3AED","Procedures":"#92400E"
    };
    return (
      <div style={{fontFamily:"'DM Sans',sans-serif",background:"#F4F7FA",minHeight:"100vh"}}>
        ...
        {/* Header */}
        <header style={{background:"linear-gradient(135deg,#0D2137,#1A3A5C)",padding:"0 28px",display:"flex",alignItems:"center",justifyContent:"space-between",height:64,boxShadow:"0 2px 12px rgba(0,0,0,.25)",position:"sticky",top:0,zIndex:100}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <button onClick={function(){setStep("home");}} style={{padding:"6px 14px",borderRadius:8,border:"2px solid rgba(255,255,255,.4)",background:"transparent",color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>← Home</button>
            <div>
              <span style={{fontFamily:"'DM Serif Display',serif",fontSize:22,color:"#fff"}}>RadReport Pro</span>
              <span style={{fontSize:11,color:"rgba(255,255,255,.45)",letterSpacing:2,textTransform:"uppercase",display:"block",marginTop:-2}}>Saved Drafts Library</span>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{padding:"6px 14px",borderRadius:20,background:"rgba(255,255,255,.1)",color:"rgba(255,255,255,.8)",fontSize:12,fontWeight:700}}>
              {savedReports.length} {savedReports.length===1?"Draft":"Drafts"} Saved
            </div>
          </div>
        </header>
        ...
        <div style={{maxWidth:900,margin:"0 auto",padding:"32px 20px 60px"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
            <div>
              <h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:28,color:"#0D2137",margin:"0 0 6px"}}>📁 Saved Drafts</h2>
              <p style={{fontSize:14,color:"#5A7090",margin:0}}>All your saved reports — click any card to load and continue editing.</p>
            </div>
            {savedReports.length > 0 && (
              <button
                onClick={function(){
                  if(window.confirm("Delete ALL " + savedReports.length + " saved drafts? This cannot be undone.")) {
                    setSavedReports([]);
                    showToast("🗑️ All drafts cleared", "info");
                  }
                }}
                style={{padding:"8px 16px",borderRadius:8,border:"2px solid #FCA5A5",background:"transparent",color:"#B91C1C",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",flexShrink:0}}
              >🗑️ Clear All</button>
            )}
          </div>
          ...
          {/* Empty state */}
          {savedReports.length === 0 && (
            <div style={{textAlign:"center",padding:"80px 20px",background:"#fff",borderRadius:16,border:"2px dashed #DDE5EF"}}>
              <div style={{fontSize:56,marginBottom:16}}>📋</div>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:22,color:"#0D2137",marginBottom:8}}>No Saved Drafts Yet</div>
              <div style={{fontSize:14,color:"#5A7090",marginBottom:28,maxWidth:340,margin:"0 auto 28px"}}>While filling a report, click <strong>"💾 Save Draft"</strong> to save your work and come back to it any time.</div>
              <button onClick={function(){setStep("home");}} style={{padding:"11px 28px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#0077B6,#00B4D8)",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Start a New Report</button>
            </div>
          )}
          ...
          {savedReports.length > 0 && (
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}}>
              {savedReports.map(function(draft, idx) {
                var mc = modalityColors[draft.modality] || "#0077B6";
                var filledCount = Object.values(draft.findings||{}).filter(function(v){return v&&v.trim();}).length;
                var abnCount = Object.values(draft.tags||{}).filter(function(t){return t==="ab";}).length;
                var hasImp = !!(draft.impression && draft.impression.trim());
                return (
                  <div key={draft.id} style={{background:"#fff",borderRadius:14,boxShadow:"0 2px 16px rgba(0,0,0,.07)",overflow:"hidden",border:"1px solid #E8EEF6",transition:"all .2s",display:"flex",flexDirection:"column"}}>
                    {/* Colour bar */}
                    <div style={{height:5,background:"linear-gradient(90deg,"+mc+","+mc+"88")} } />

                    <div style={{padding:"18px 20px 16px",flex:1}}>
                      {/* Label - editable */}
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:8}}>
                        <div style={{fontWeight:700,fontSize:15,color:"#0D2137",lineHeight:1.3,flex:1}}>{draft.label}</div>
                        <button
                          title="Rename"
                          onClick={function(){
                            var nl = window.prompt("Rename this draft:", draft.label);
                            if(nl && nl.trim()) updateDraftLabel(draft.id, nl.trim());
                          }}
                          style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:"#8BA0B8",flexShrink:0,padding:"2px 4px",borderRadius:4}}
                        >✏️</button>
                      </div>

                      {/* Modality + Region pill */}
                      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
                        <span style={{padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700,background:mc+"18",color:mc,border:"1px solid "+mc+"33"}}>{draft.modality||"—"}</span>
                        {draft.region && <span style={{padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600,background:"#F0F4FF",color:"#4F46E5",border:"1px solid #C7D2FE"}}>{draft.region}</span>}
                      </div>

                      {/* Patient info */}
                      {draft.patient && draft.patient.name && (
                        <div style={{fontSize:12,color:"#5A7090",marginBottom:6}}>
                          <span style={{fontWeight:600}}>👤 {draft.patient.name}</span>
                          {draft.patient.age && <span style={{marginLeft:6}}>{draft.patient.age}y, {draft.patient.sex}</span>}
                        </div>
                      )}

                      {/* Stats row */}
                      <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
                        <span style={{fontSize:11,color:"#5A7090"}}>📝 {filledCount} field{filledCount!==1?"s":""}</span>
                        {abnCount > 0 && <span style={{fontSize:11,color:"#B91C1C",fontWeight:700}}>⚠️ {abnCount} abnormal</span>}
                        {hasImp && <span style={{fontSize:11,color:"#2D9E6B",fontWeight:700}}>✅ Impression</span>}
                        {draft.urgency && draft.urgency !== "Routine" && (
                          <span style={{fontSize:11,fontWeight:700,color:draft.urgency==="Urgent"?"#E07B39":"#B91C1C"}}>🔴 {draft.urgency}</span>
                        )}
                      </div>

                      {/* Saved time */}
                      <div style={{fontSize:11,color:"#B0BEC5"}}>🕐 {fmtDate(draft.savedAt)}</div>
                    </div>

                    {/* Action buttons */}
                    <div style={{display:"flex",borderTop:"1px solid #F0F4F8"}}>
                      <button
                        onClick={function(){ loadDraft(draft); }}
                        style={{flex:1,padding:"12px 0",border:"none",background:"none",color:mc,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",borderRight:"1px solid #F0F4F8",transition:"background .15s"}}
                        onMouseEnter={function(e){e.currentTarget.style.background=mc+"12";}}
                        onMouseLeave={function(e){e.currentTarget.style.background="none";}}
                      >📂 Open &amp; Edit</button>
                      <button
                        onClick={function(){
                          if(window.confirm("Delete "" + draft.label + ""?")) deleteDraft(draft.id);
                        }}
                        style={{width:48,padding:"12px 0",border:"none",background:"none",color:"#B0BEC5",fontWeight:700,fontSize:15,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all .15s"}}
                        onMouseEnter={function(e){e.currentTarget.style.background="#FEF2F2";e.currentTarget.style.color="#B91C1C";}}
                        onMouseLeave={function(e){e.currentTarget.style.background="none";e.currentTarget.style.color="#B0BEC5";}}
  ...
  if (step === "home") return (
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#04090F",minHeight:"100vh",overflow:"hidden",position:"relative",color:"#fff"}}>
      ...
      <nav className="np" style={{position:"relative",zIndex:20,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"22px 40px",borderBottom:"1px solid rgba(56,189,248,.08)"}}>
        ...
        {/* Nav right: status pills + saved drafts button */}
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          {/* Saved Drafts entry point */}
          <button
            onClick={function(){setStep("saved");}}
            style={{display:"flex",alignItems:"center",gap:8,padding:"8px 18px",borderRadius:24,background:"rgba(79,70,229,.18)",border:"1px solid rgba(99,102,241,.4)",cursor:"pointer",backdropFilter:"blur(8px)",transition:"all .2s",fontFamily:"'DM Sans',sans-serif"}}
            onMouseEnter={function(e){e.currentTarget.style.background="rgba(79,70,229,.35)";}}
            onMouseLeave={function(e){e.currentTarget.style.background="rgba(79,70,229,.18)";}}
          >
            <span style={{fontSize:14}}>📁</span>
            <span style={{fontSize:11,color:"#A5B4FC",fontWeight:700,letterSpacing:"1px"}}>SAVED DRAFTS</span>
            {savedReports.length > 0 && (
              <span style={{minWidth:18,height:18,borderRadius:9,background:"#4F46E5",color:"#fff",fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 5px"}}>{savedReports.length}</span>
            )}
          </button>
        </div>
      </nav>
      ...
      <div style={{position:"relative",zIndex:10,padding:"0 40px 100px"}}>
        ...
        {/* Saved Drafts shortcut strip */}
        <div
          onClick={function(){setStep("saved");}}
          style={{marginTop:20,padding:"18px 28px",borderRadius:14,border:"1px solid rgba(99,102,241,.25)",background:"rgba(79,70,229,.08)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",transition:"all .2s",animation:"fadeUp .6s ease 1s both",flexWrap:"wrap",gap:12}}
          onMouseEnter={function(e){e.currentTarget.style.background="rgba(79,70,229,.18)";e.currentTarget.style.borderColor="rgba(99,102,241,.5)";}}
          onMouseLeave={function(e){e.currentTarget.style.background="rgba(79,70,229,.08)";e.currentTarget.style.borderColor="rgba(99,102,241,.25)";}}
        >
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#4F46E5,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,boxShadow:"0 4px 16px rgba(79,70,229,.4)"}}>📁</div>
            <div>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:18,color:"#fff",lineHeight:1.2}}>Saved Drafts</div>
              <div style={{fontSize:12,color:"rgba(255,255,255,.4)",marginTop:2}}>
                {savedReports.length === 0
                  ? "No saved drafts yet — save any report to continue later"
                  : savedReports.length + " saved report" + (savedReports.length !== 1 ? "s" : "") + " — click to open your library"
                }
              </div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
            {savedReports.length > 0 && savedReports.slice(0,3).map(function(d){
              var mc = ({"Ultrasound":"#0077B6","X-Ray":"#047857","CT Scan":"#B45309","MRI":"#6D28D9"})[d.modality] || "#4F46E5";
              return (
                <div key={d.id} style={{padding:"4px 10px",borderRadius:20,background:mc+"25",border:"1px solid "+mc+"40",fontSize:10,color:"rgba(255,255,255,.65)",fontWeight:700,whiteSpace:"nowrap"}}>
                  {d.label.length > 20 ? d.label.slice(0,20)+"…" : d.label}
                </div>
              );
            })}
            {savedReports.length > 3 && <div style={{padding:"4px 10px",borderRadius:20,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",fontSize:10,color:"rgba(255,255,255,.4)",fontWeight:700}}>+{savedReports.length-3} more</div>}
            <div style={{fontSize:11,color:"#A5B4FC",fontWeight:700,letterSpacing:".5px",display:"flex",alignItems:"center",gap:4,marginLeft:4}}>OPEN LIBRARY →</div>
          </div>
        </div>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,minHeight:"100vh"}}>
      ...
      <AppHdr onBack backTo="patient" setStep={setStep} sub={modality+" › "+region+" › Findings"}
        right={<div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <button
            onClick={function(){setSaveModalOpen(true);}}
            style={{padding:"7px 16px",borderRadius:8,border:"2px solid rgba(99,102,241,.6)",background:"rgba(79,70,229,.2)",color:"#A5B4FC",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",gap:6}}
          >💾 Save Draft</button>
